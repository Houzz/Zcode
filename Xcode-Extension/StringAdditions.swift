            ...
            case .token:
                if let token = scanner.scanUpTo(string: "}") {
                    if let argDict = argDict {
                        ...
                        if idx < args.count {
                            output += String(describing: args[idx - 1] ?? "")
                        }
                    } else {
...
public extension String {
    ...
    func stringWithNoWhitespaces() -> String? {
 ...
 extension String {
    ...
    public var deleteLastPathComponent: String {
    ...
    public func append(pathComponent str: String) -> String {
        return hasSuffix("/") ? "\(self)\(str)" : "\(self)/\(str)"
    }

    /// add path extension
    public func append(pathExtension ext: String) -> String {
        return hasSuffix(".") ? "\(self)\(ext)" : "\(self).\(ext)"
    }

    /// Delete path extension
    public var deletePathExtension: String {
        if let idx = self.range(of: ".", options: .backwards, range: self.startIndex ..< self.endIndex, locale: nil)?.lowerBound {
            return String(self[..<idx])
        }
        return self
